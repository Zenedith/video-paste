...
//    console.log("Database_Redis: Redis Error: " + err);
  ...
  this._createSetNameKey = function (classname, id, setName) {
    return this.prefix + ':' + classname + ':' + id + ':' + setName;
  };
...
Database_Redis.prototype.getObjectsFromSet = function (obj, setName, callback) {
  log.debug('Database_Redis.getObjectsFromSet()');

  var
    classname = obj.getClassName();

  this.client.smembers(this._createSetNameKey(classname, obj.getId(), setName), callback);
};

Database_Redis.prototype.isObjectInSet = function (obj, setName, value, callback) {
  log.debug('Database_Redis.isObjectInSet()');

  var
    classname = obj.getClassName();

  this.client.sismember(this._createSetNameKey(classname, obj.getId(), setName), value, callback);
};

Database_Redis.prototype.addToObjectSet = function (obj, setName, value, callback) {
  log.debug('Database_Redis.addToObjectSet()');

  var
    classname = obj.getClassName();

  this.client.sadd(this._createSetNameKey(classname, obj.getId(), setName), value, callback);
};
