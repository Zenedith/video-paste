...
//    console.log("Database_Redis: Redis Error: " + err);
    ...
    this._createKey = function (key) {
        return this.prefix + ':' + key;
    };

    this._createHashKey = function (key) {
        return this.prefix + ':' + key;
    };
...
Database_Redis.prototype.getManyValuesFromLists = function (listNames, start, stop, callback) {
    ...
    for (var lp in listNames) {
        if (listNames.hasOwnProperty(lp)) {
...
Database_Redis.prototype.getSortedValuesFromSetByString = function (setName, offset, count, order, callback) {
    log.debug('Database_Redis.getSortedValuesFromSet(' + setName + ', ' + offset + ', ' + count + ', ' + order + ')');

    this.client.sort(this._createSetNameKey(setName), 'by', 'alpha', 'limit', offset, count, order, callback);
};
...
Database_Redis.prototype.set = function (key, value, callback) {
    log.debug('Database_Redis.set(' + key + ')');

    this.client.set(this._createKey(key), value, callback);
};

Database_Redis.prototype.get = function (key, callback) {
    log.debug('Database_Redis.get(' + key + ')');

    this.client.get(this._createKey(key), callback);
};

Database_Redis.prototype.setValues = function (key, values, callback) {
    log.debug('Database_Redis.setValues(' + key + ')');

    this.client.hmset(this._createHashKey(key), values, callback);
};

Database_Redis.prototype.getValues = function (key, callback) {
    log.debug('Database_Redis.getValues(' + key + ')');
    log.debug(this._createHashKey(key));

    this.client.hgetall(this._createHashKey(key), callback);
};


Database_Redis.prototype.quit = function () {
    this.client.quit();
};
                        ...
                        if (batchData[lp][type].hasOwnProperty(lpp)) {
                            ...
                            switch (type) {
                                ...
                                case 'lrange':
                                    ...
                                    break;
                                case 'hmset':
                                    key = this._createHashKey(data[0]);
                                    row.push(key);
                                    row.push(data[1]);
                                    break;
                                case 'hgetall':
...
Database_Redis.prototype.setObjectValue = function (obj, field, value, callback) {
    ...
    log.debug(this._createFieldKey(classname, obj.getId(), field));
    log.debug(value);
...
Database_Redis.prototype.loadManyObjects = function (obj, ids, callback) {
    ...
    for (var lp in ids) {
        if (ids.hasOwnProperty(lp)) {
            ...
            for (var k in objKeys) {
                if (objKeys.hasOwnProperty(k) && typeof(objKeys[k]) !== "function") {
